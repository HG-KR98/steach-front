...
export const startAlarm = (type: "comeback" | "sleep") => {
  ...
  alarmTextElement.textContent =
    type === "comeback" ? "화면에 집중하세요!" : "일어나세요!";
  ...
  imgElement.animate(
    type === "comeback" ? animationForAFK : animationForSleep,
    {
  ...
  if (!audioElement) {
    ...
    audioElement
      .play()
      .catch((error) => console.error("Audio play error:", error)); // Catch any play errors
  }
};
    ...
    socketRef.current.on(
      ...
      (data: {
        ...
        if (data.screenShareDisabledByTeacher) {
          if (data.userId === socketRef.current?.id) {
            setIsScreenShareDisabledByTeacher(
              data.screenShareDisabledByTeacher
            );
            setIsScreenShareEnabled(false);
          }
